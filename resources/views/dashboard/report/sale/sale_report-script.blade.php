<script>
  ...
  $('body').on('submit', '#form_select', function (e) {
    ...
    $('#table_').DataTable({
      ...
      columnDefs: [
        ...
        { targets: [1, 3], className: "text-nowrap align-middle text-capitalize" },
        { targets: [2], className: "text-nowrap align-middle text-center text-capitalize" },
      ]
    });
    ...
    if($('#report_year').val() != '' && $('#report_month').val() != '') {
      $('#row_table_title').text(`Data Penjualan Pada Bulan ${getMonthName($('#report_month').val())} Tahun ${$('#report_year').val()}`);
    }else if($('#report_year').val() != '') {
      $('#row_table_title').text(`Data Penjualan Pada Tahun ${$('#report_year').val()}`);
    }else {
      $('#row_table_title').text('Data Semua Penjualan');
    }
  })
  ...
  $('body').on('click', '.btn-print', function (e) {
    ...
    var newURL = 'sale/export?year=' + year + '&month=' + month;
