<script>
  ...
  $('body').on('submit', '#form_select', function (e) {
    ...
    $('#table_').DataTable({
      ...
      columnDefs: [
        ...
        { targets: [1, 2, 5, 6, 7], className: "text-nowrap align-middle text-capitalize" },
    ...
    if($('#report_year').val() != '' && $('#report_month').val() != '') {
      $('#row_table_title').text(`Data Pembelian Bahan Pada Bulan ${getMonthName($('#report_month').val())} Tahun ${$('#report_year').val()}`);
    }else if($('#report_year').val() != '') {
      $('#row_table_title').text(`Data Pembelian Bahan Pada Tahun ${$('#report_year').val()}`);
    }else {
      $('#row_table_title').text('Data Semua Pembelian Bahan');
    }
  })
  ...
  $('body').on('click', '.btn-print', function (e) {
    ...
    var newURL = 'purchase/export?year=' + year + '&month=' + month;
