<script>
  ...
  $('body').on('submit', '#form_select', function (e) {
    ...
    if($('#report_year').val() != '' && $('#report_month').val() != '') {
      $('#row_table_title').text(`Data Pesanan offline Pada Bulan ${getMonthName($('#report_month').val())} Tahun ${$('#report_year').val()}`);
    }else if($('#report_year').val() != '') {
      $('#row_table_title').text(`Data Pesanan offline Pada Tahun ${$('#report_year').val()}`);
    }else {
      $('#row_table_title').text('Data Semua Pesanan offline');
    }
  })
  ...
  $('body').on('click', '.btn-print', function (e) {
    ...
    var newURL = 'offline-order/export?year=' + year + '&month=' + month;
