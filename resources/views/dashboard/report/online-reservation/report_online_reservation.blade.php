  ...
  <div class="body">
    ...
    <div class="header">
      ...
      <p id="title" class="text-center">
        Laporan Reservasi Online
      </p>
      @else
      <p id="title" class="text-center">
        Laporan Semua Reservasi Online
      </p>
      @endif
    ...
    <div class="row">
      <table>
        ...
        <tr>
          ...
          <td width="150px" style="text-align: left; text-transform: capitalize;">{{ $item->reservation_id }}</td>
          ...
          <td width="75px">{{ formatTime($item->reservation_time) }}</td>
          <td width="60px">{{ ($item->visit_time) ? formatTime($item->visit_time) : '-' }}</td>
          <td width="60px">{{ ($item->finished_time) ? formatTime($item->finished_time) : '-' }}</td>
          <td width="85px">{{ $item->reservation_status }} </td>
          <td width="160px"
            style="text-align: @if($item->online_order) left @else center @endif; text-transform: capitalize;">
            @if ($item->online_order)
            {{ $item->online_order->online_order_id }}
            @else
            -
            @endif
          </td>
        ...
        @endforeach
        <tr>
          <th colspan="7">Total Reservasi Status Pending</th>
          <th colspan="2">{{ ribu($pending) }} reservasi</th>
        </tr>
        <tr>
          <th colspan="7">Total Reservasi Status Process</th>
          <th colspan="2">{{ ribu($process) }} reservasi</th>
        </tr>
        <tr>
          <th colspan="7">Total Reservasi Status Success</th>
          <th colspan="2">{{ ribu($success) }} reservasi</th>
        </tr>
        <tr>
          <th colspan="7">Total Reservasi Status Failed</th>
          <th colspan="2">{{ ribu($failed) }} reservasi</th>
        </tr>
        <tr>
          <th colspan="7">Total Semua Reservasi</th>
          <th colspan="2">{{ ribu(count($reservation)) }} reservasi</th>
        </tr>
      </table>
    </div>
