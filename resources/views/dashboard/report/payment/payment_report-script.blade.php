<script>
  ...
  $('body').on('submit', '#form_select', function (e) {
    ...
    $('#table_').DataTable({
      ...
      columnDefs: [
        ...
        { targets: [1, 5, 6], className: "text-nowrap align-middle text-capitalize" },
    ...
    if($('#report_year').val() != '' && $('#report_month').val() != '') {
      $('#row_table_title').text(`Data Pembayaran Pada Bulan ${getMonthName($('#report_month').val())} Tahun ${$('#report_year').val()}`);
    }else if($('#report_year').val() != '') {
      $('#row_table_title').text(`Data Pembayaran Pada Tahun ${$('#report_year').val()}`);
    }else {
      $('#row_table_title').text('Data Semua Pembayaran');
    }
  })
  ...
  $('body').on('click', '.btn-print', function (e) {
    ...
    var newURL = 'payment/export?year=' + year + '&month=' + month;
