    ...
    <div class="row">
      <table>
        ...
        <tr>
          ...
          <td width="150px" style="text-align: left; text-transform: capitalize;">{{ $item->payment_id }}</td>
          <td style="text-align: left; text-transform: capitalize;">
            @if ($item->order)
            {{ $item->order->name }}
            @else
            {{ $item->offline_order->name }}
            @endif
          </td>
          <td width="163px">
            {{ formatDateWithTime($item->created_at) }}
          </td>
          <td width="118px">{{ $item->payment_status }} </td>
          <td width="130px" style="text-align: left; text-transform: capitalize;">
            @if ($item->order)
            {{ rupiah($item->order->total) }}
            @else
            {{ rupiah($item->offline_order->total) }}
            @endif
          </td>
          <td width="150px" style="text-align: left; text-transform: capitalize;">
            @if ($item->order)
            {{ $item->order->online_order_id }}
            @else
            {{ $item->offline_order->offline_order_id }}
            @endif
          </td>
        </tr>
        @endforeach
        <tr>
          <th colspan="5">Total Pembayaran Status Pending</th>
          <th colspan="2">{{ ribu($pending) }} pembayaran</th>
        </tr>
        <tr>
          <th colspan="5">Total Pembayaran Status Expired</th>
          <th colspan="2">{{ ribu($expired) }} pembayaran</th>
        </tr>
        <tr>
          <th colspan="5">Total Pembayaran Status Failed</th>
          <th colspan="2">{{ ribu($failed) }} pembayaran</th>
        </tr>
        <tr>
          <th colspan="5">Total Pembayaran Status Success</th>
          <th colspan="2">{{ ribu($success) }} pembayaran</th>
        </tr>
        <tr>
          <th colspan="5">Total Semua Pembayaran</th>
          <th colspan="2">{{ ribu(count($payment)) }} Pembayaran</th>
        </tr>
        <tr>
